...
export default function ImageUpload({ currentImage, onImageChange, className = "" }) {
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      <Button
        onClick={handleImageClick}
        className="mt-3 px-6 py-2 text-sm rounded-full hover:scale-105 transition-all duration-200 shadow-md hover:shadow-lg flex items-center gap-2"
        style={{ 
          backgroundColor: '#b9d7d9',
          color: '#7b3b3b',
          border: '2px solid #f9f2e0'
        }}
      >
        <Upload size={16} />
        Upload Photo ✨
